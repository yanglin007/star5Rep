...
const XItem = ({ item, delItem, pitem, pid }) => {
    return (
        <div className='xItem'>
            <span>{item.name}</span>
            <CloseOutlined onClick={() => delItem(item, pitem, pid)} />
            ...
            <Row>
                ...
                <Col span={9}>
                    <Popover placement="bottom" content={content} trigger="click">
                        <span style={{ position: 'relative', paddingLeft: '20px' }}>  <span className='colorLabel' style={{ background: item.color }}></span>  color</span>
                    </Popover>
                    <CloseOutlined onClick={() => delItem(item, pitem, pid)} />
                </Col>
            </Row>
