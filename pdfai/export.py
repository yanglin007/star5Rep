...
def chatChart(df, config):
    chat = ChatOpenAI(temperature=0)
    agent = create_pandas_dataframe_agent(chat, df, verbose=True)
    dt = agent.run(config.get("prompt"))
    print('------dt----',dt, type(dt))
...
def export_excel(df,config):
    target_path = './mydata//'+config.get("file_name")+".xlsx"
...
def data_to_pie(df,config):
    ...
    chat = ChatOpenAI(temperature=0)
    agent = create_pandas_dataframe_agent(chat, df, verbose=True)
    dt = agent.run(config.get("desc")+",map to which coloumns")
    print('------dt----',dt, type(dt))
