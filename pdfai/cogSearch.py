...
def saveWf(request):
    ...
    if max_wf_id['work_flow_id__max'] is None:
        wf_id=0
    else :
        wf_id = max_wf_id["work_flow_id__max"]
    i = 0
    for item in libs:
        print("test=====",item,type(item))
        i = i +1
        wf = Workflow(work_flow_id = wf_id ,order_num =i,work_flow_name=wf_name,
...
def runWf(request):
    ...
    if wf_id != None: # run exised work flow
        ...
        for lib in libs:
            config = lib["config"]
            item = Libs.objects.get(lib_id = lib['lib_id'])
            print('ttt=====',lib,type(item),item)
            if i==0:
                previous = eval(item.lib_func)(None,config)
                i = i +1
            previous = eval(item.lib_func)(previous,config)
